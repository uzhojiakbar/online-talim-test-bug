import React, { useState } from 'react'
import { delCookie } from '../../../Hooks/getCooce';
import { useNavigate } from 'react-router-dom';
import { Dropdown } from 'antd';
function AdminNav() {
    const name = JSON.parse(localStorage.getItem('myArray'))
    const [open, setOpen] = useState(false)

    const nav = useNavigate();
    const delestate = () => {
        delCookie('token')
        nav('/')
        document.location.reload()
    }
    const menu = (
        <div className='p-4 bg-slate-white shadow-lg bg-white'>
            <span onClick={delestate} className='flex items-center gap-2 hover:bg-slate-100 p-2 rounded-sm'>
                <h1 className='cursor-pointer'>chiqish</h1>
                <i class="fa-solid fa-right-from-bracket"></i>
            </span>
            <span className='flex items-center gap-2 hover:bg-slate-100 rounded-sm p-2 rounded-sm'>
                <h1 className='cursor-pointer'>Sersifikat olish </h1>
            </span>
        </div>
    )

    return (
        <div className=' max-md:pl-16 bg-white z-[2000] py-2 px-8 w-full flex  justify-between fixed items-center border-b'>
            <h1 className="text sm:text-xxl text-xl font-bold">Admin panel</h1>
            <Dropdown overlay={menu}>

                <div className='flex gap-2 sm:text-xl text-xl font-bold text-slate-800 items-center'>
                    <p >{name?.firstname}</p>
                    <img onClick={() => setOpen(!open)} className='sm:w-12 w-12' src="data:image/png;base64," alt="" />
                </div>

            </Dropdown>




        </div>
    )
}

export default AdminNav
