...
function Homesection1() {
    ...
    const { fan } = useFan();
    const showMore = () => setCount((prev) => prev + 3);
    const showLess = () => setCount(6);
    ...
    return (
        <div id="darslar" className="lg:p-16 px-4 overflow-hidden">
            ...
            <div data-aos="zoom-in" data-aos-duration="500" className="flex justify-center mt-12">
                {SecCarddata.length > 6 && count < SecCarddata.length && (
                    <button 
                        onClick={showMore} 
                        className="border border-[#828080] rounded-lg px-12 py-3 hover:bg-[#FF6E30] hover:text-white transition-all text-[#FF6E30] font-[500]"
                    >
                        Yana ko'rsatish
                    </button>
                )}
                {count >= SecCarddata.length && (
                    <button 
                        onClick={showLess} 
                        className="border border-[#828080] rounded-lg px-12 py-3 hover:bg-[#FF6E30] hover:text-white transition-all text-[#FF6E30] font-[500]"
                    >
